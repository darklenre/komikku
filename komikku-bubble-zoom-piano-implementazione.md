...
| 4 | **Sezione "Bubble Zoom" nelle impostazioni Lettore** — ✅ fatto (Antigravity) | `SettingsReaderScreen.getBubbleZoomGroup()` (categoria dedicata: `SwitchPreference` attivazione `bubbleZoom()`, `enabled = supported`); voce vecchia rimossa da `getActionsGroup`. Selettore motore ONNX rimosso (motore unico TFLite). |
| 5 | **Motore ML unico: TFLite + LiteRT (ONNX dismesso)** — ✅ fatto (Antigravity) | Rimosso runtime ONNX (-44 MB). `bubble_detector.tflite` modello unico int8 con GPU delegate (fallback CPU XNNPACK) + `BubblePostprocess` + keep rules ProGuard. |
