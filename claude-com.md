...
- **30 ago — Claude**: **Fase 4.1 — swap EdgeSAM** (scheda I/O Antigravity in `antigravity-com.md`). `SamRefiner`: asset `sam_*_edgesam.tflite` + SHA nuovi, vecchi MobileSAM rimossi (`git rm`), `NOTICE` → EdgeSAM (NTU S-Lab, Apache-2.0). Shape/dtype/normalizzazione identici → swap drop-in, nessun refactor. **GPU delegate forzato** oltre il falso-negativo di `CompatibilityList` → encoder EdgeSAM su GPU **~150-180 ms** sul Fold8 (vs ~800 CPU, ~2000 MobileSAM). Decoder → fallback CPU pulito (GPU rifiuta un `CONCAT` 3D). **Fix crash SIGSEGV**: `SamRefiner` legge lo stream pagina 1 volta in `ByteArray` e decodifica con `decodeByteArray` (non `decodeStream` sullo stream vivo — il prewarm concorrente lo corrompeva, native, non catchabile). Log WARN diagnostici (`EdgeSAM ready`, `encode Nms`).
- **30 ago — Claude**: **estrazione balloon a due livelli** (dopo ricerca: i tool reali usano flood-fill classico, non reti; CNN dedicate ~0.94 F1, licenze incerte). **Tier-1** `BubbleFloodExtractor` (nuovo): flood-fill dell'interno chiaro delimitato dall'inchiostro + hole-fill del testo + gate di validazione (area/solidità/copertura/border-leak) → maschera esatta o `null`. **Tier-2** EdgeSAM per-lobo + **edge-snap** guided-filter (`snapToEdges`, guida = grayscale crop) sull'output SAM. **Merge lobi collegati**: `groupLinked` (union-find, stacked *o* side-by-side, gap stretto) + `mergeLinked` → `Bubble.lobes`; applicato in `bubbleDetections()` dei viewer (detection resta per-lobo, hit-test preciso). `BubbleExtractor.compositeMask`: estrae **ogni lobo separato** (Tier-1→Tier-2) e fa l'**OR** → niente componente monca né clipping né box largo che confonde il modello. `CROP_MARGIN` 0.22→0.26, pad SAM 0.14→0.16, blur-threshold rimosso. 40 unit test (`BubbleGroupingTest` nuovo). Utente: singole/coppie/3-stack **ottime** su pag.18 cap.1 Absolute Superman. Commit — **cristallizza v5**.
- **30 ago — Claude**: tentativi v6/v7 (multi-contorno `traceAllContours`, `bridgeGaps` close morfologico, clamp `snapToEdges`, `BOUND_GROW` 0.32 + reject perimetro) → **regressioni** (3-stack diventano 3 blob staccati con outline spesso). **Rollback a v5** su richiesta utente. **Difetti noti v5 aperti**: (1) outline perso su lobo disconnesso — serve multi-contorno *senza* separare i lobi realmente connessi; (2) bordo del pannello a volte nel cutout quando il balloon è appoggiato alla cornice.
- **30 ago — Claude**: iterazione qualità cutout (dump di debug `Log.isLoggable("BZDump", VERBOSE)` → PNG di ogni stadio in `getExternalFilesDir("bzdump")`). **A5**: colla union-SAM confinata alle *bridge-zone* tra lobi adiacenti + multi-contorno (`traceAllContours`, un subpath per blob). **B3**: `clipToLobeRegion` — clip a rounded-rect (union lobi +10%, angoli 30%) contro cornice/gutter di pannello. **B4**: clamp allow-band su `snapToEdges` (guided filter confinato a ~2r della maschera SAM). **Merge content-aware**: `mergeLinked(bubbles, pageBitmap)` spostato in `BubbleDetection.detect` (ha la bitmap); `groupLinked` (geometria, allentata 0.40/0.35) *propone*, `corridorIsFill` conferma guardando se la striscia tra i lobi è fill-balloon (luma≥175, sat≤55) sulla bitmap reale → distingue balloon pinchato da due balloon adiacenti coi box che si sovrappongono. `mergeLinked` rimosso dai viewer. Dai dump: fv8-b (MY WORLD?+AND I'VE SEEN) = due balloon separati con disegno scuro in mezzo → split; fv8-c (YOU KNOW+TRUTH) = balloon unico pinchato → merge; 3-stack = balloon unico → merge. compile+41 test+spotless ✅, APK Fold8. Da validare a runtime.
