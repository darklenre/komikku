...
**Da verificare a runtime sul device** (serve interazione reader): IN_PLACE/FLOATING, hit-test ai bordi pagina, page-turn tra bubble, aspetto outline vettoriale + slider spessore, barra solo su encode SAM interattivo, toast "Detecting…".

**Fase 0 (roadmap piano §2)** — in corso dopo questo commit: togliere log INFO diagnostici · gate device adattivo · SHA-256 modelli al load + release su `onTrimMemory` · cancel encode SAM stantio al page-turn · unit test funzioni pure.
